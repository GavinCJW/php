	...
	private function setContent(&$worksheet,&$dbData,&$exportDataDes){
		...
		foreach ($dbData as $key => $val){
			foreach ( $exportDataDes['fieldKey'] as $k => $v ) {
				if (isset($val[$v])){
					if (isset($exportDataDes['fieldFormat'][$k])){
						switch ($exportDataDes['fieldFormat'][$k]){
							case 'TEXT':
								$selectCell = $this->getColumnAlph($k).$i;
								$worksheet->setCellValueExplicit($selectCell,$val[$v],PHPExcel_Cell_DataType::TYPE_STRING);
								$worksheet->getStyle($selectCell)->getNumberFormat()->setFormatCode("@");
								break;
							default:
